...
from scraper.spiders.ges import GESSpider


class DiscloseDREALGESScraper(AddOn):
    """Scraper for DREAL GES documents (https://www.grand-est.developpement-durable.gouv.fr)."""
    ...
    def check_permissions(self):
        ...
        try:
            ...
            subject = f"DREAL GES Scraper - Error connecting to DocumentCloud"
            content = f"Error:\n {e.__traceback__}"
    ...
    def main(self):
        ...
        self.client.session.headers.update(
            {"User-Agent": "Disclose GES Scraper Add-On"}
        )
        ...
        self.set_message(
            f"Scraping DREAL GES documents {str(self.target_year)} [{self.run_name}]"
        )
        process.start()
